...
async def _stream_chat_completion(payload: WebSearchRequest) -> AsyncIterator[str]:
    created = int(time.time())
    ...
    try:
        execution = await search_service.execute(payload)
...
async def create_chat_completion(payload: WebSearchRequest) -> Any:
    if payload.stream:
        try:
...
@router.post("/create_task", dependencies=[Depends(require_auth)])
async def create_task(payload: WebSearchRequest) -> Dict[str, Any]:
    try:
        extract_query(payload)
    except ValueError as exc:
        raise HTTPException(
    ...
    task = await task_store.create(payload)
    return {
        "code": 0,
        "message": "OK",
        "data": {
...
@router.get("/query_task", dependencies=[Depends(require_auth)])
async def query_task_get(task_id: str = Query(..., min_length=1)) -> Any:
...
@router.post("/query_task", dependencies=[Depends(require_auth)])
async def query_task_post(payload: TaskQueryRequest) -> Any:
