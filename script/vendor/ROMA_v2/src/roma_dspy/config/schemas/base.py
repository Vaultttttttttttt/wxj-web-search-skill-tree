...
class LLMConfig:
    ...
    max_tokens: int = 2000
    timeout: int = 120  # 2 minutes per HTTP request; retries handle transient failures
...
class RuntimeConfig:
    ...
    parallel_write_chapters: bool = True  # WRITE chapters execute in parallel (depend only on outline THINK)
    skip_content_filter_badrequest: bool = True  # Tolerate provider-side DataInspectionFailed by skipping only the affected subtask
