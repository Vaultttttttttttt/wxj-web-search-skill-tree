    ...
    def load_config(
        ...
        if overrides:
            if isinstance(overrides, dict):
                # Handle dictionary overrides (e.g. from API)
                override_config = OmegaConf.create(overrides)
                logger.debug(f"Applied dict overrides: {overrides}")
            else:
                # Handle list of strings (e.g. from CLI)
                override_config = OmegaConf.from_dotlist(overrides)
                logger.debug(f"Applied dotlist overrides: {overrides}")
