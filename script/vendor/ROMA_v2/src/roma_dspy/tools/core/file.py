...
class FileToolkit(BaseToolkit):
    ...
    def _initialize_tools(self) -> None:
        ...
        self.enable_save = self.config.get("enable_save", True)
    ...
    def _is_tool_available(self, tool_name: str) -> bool:
        ...
        if tool_name == "delete_file" and not self.enable_delete:
            return False
        if tool_name == "save_file" and not self.enable_save:
            return False
        return True
    ...
    def batch_read_files(self, file_paths: List[str]) -> str:
        """
        Read multiple files at once and return all their contents in a single response.

        Use this tool when you need to load several files in one step instead of
        calling read_file repeatedly. This is critical for WRITE tasks that need
        to load many evidence files — a single batch call avoids ReAct trajectory
        inflation that would occur with sequential read_file calls.

        Args:
            file_paths: List of file paths to read (relative to base directory)

        Returns:
            JSON string with a results dict keyed by file path. Each entry
            contains the file content on success or an error message on failure.
            Also includes a summary with total/succeeded/failed counts.

        Examples:
            batch_read_files(["artifacts/retrieve_india_pds.md",
                              "artifacts/retrieve_mgnrega.md"])
        """
        if not file_paths:
            return json.dumps(
                {"success": False, "error": "file_paths list is empty"}
            )
        ...
        for path in file_paths:
            try:
                ...
                if not full_path.exists():
                    results[path] = {"success": False, "error": f"File does not exist"}
                    failed += 1
                    continue

                if not full_path.is_file():
                    results[path] = {"success": False, "error": f"Not a file"}
                    failed += 1
                    continue
        ...
        self.log_debug(
            f"Batch read {len(file_paths)} files: "
