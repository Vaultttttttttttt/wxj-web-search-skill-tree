...
def _convert_mcp_tool_to_dspy(client, tool):
    ...
    async def func(*args_tuple, **kwargs):
        try:
            # Check if client is None before calling (handles reconnection issues)
            if client is None:
                raise RuntimeError(
                    f"MCP client not available for tool '{tool.name}'. "
                    "Connection may have been lost. This usually happens after VPN changes. "
                    "Please restart the application to re-establish the connection."
                )
...
class MCPToolkit(BaseToolkit):
    ...
    async def _async_initialize(self) -> None:
        ...
        # Clear old tools before re-initialization to prevent stale references
        self._mcp_tools = {}
