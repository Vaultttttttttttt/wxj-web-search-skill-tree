...
class TaskDAG:
    ...
    @property
    def root(self) -> "TaskDAG":
        """Return the root DAG by traversing parent_dag references upward."""
        dag = self
        while dag.parent_dag is not None:
            dag = dag.parent_dag
        return dag
    ...
    def add_edge(
        ...
        # Self-loop guard: a task cannot depend on itself
        if from_task_id == to_task_id:
            logger.warning(
                f"[DAG] Skipping self-loop edge: {from_task_id[:8]}... → itself"
            )
            return
    ...
    def add_dependencies(self, task_id: str, dependency_ids: List[str]) -> TaskNode:
        ...
        for dep_id in dependency_ids:
            ...
            try:
                ...
                # Update task dependencies only if edge was successfully added
                task = task.add_dependency(dep_id)
            except ValueError as e:
                logger.warning(
                    f"[DAG] Dropping invalid dependency edge {dep_id[:8]}... -> {task_id[:8]}...: {e}"
                )
    ...
    def get_execution_order(self) -> List[str]:
        ...
        Uses Kahn's BFS algorithm with ``planner_seq_idx`` as a min-heap
        tiebreaker so that sibling nodes (no relative dependency between them,
        e.g. parallel WRITE chapters) are emitted in the same order the
        planner originally listed them.  This guarantees a stable,
        deterministic chapter sequence in the assembled final report regardless
        of how NetworkX internally stores the graph.
        ...
        def _priority(node_id: str) -> int:
            ...
            if task is None:
                return 9999
            return (task.metadata or {}).get("planner_seq_idx", 9999)

        # Use a monotonically increasing counter as a secondary tiebreaker so
        # that the heap comparison never falls through to comparing node_id
        # strings (which are UUIDs and have no semantic order).
        counter = 0
        heap: list = []
        for n in self.graph.nodes():
            if in_deg[n] == 0:
                heapq.heappush(heap, (_priority(n), counter, n))
                counter += 1
        ...
        while heap:
            ...
            for successor in self.graph.successors(n):
                ...
                if in_deg[successor] == 0:
                    heapq.heappush(heap, (_priority(successor), counter, successor))
                    counter += 1

        if len(result) != self.graph.number_of_nodes():
            raise ValueError(
    ...
    def from_dict(
        ...
        for task_id, raw_task_data in tasks_data.items():
            task_data = dict(raw_task_data)

            # Backfill execution_id from DAG metadata for legacy snapshots that
            # serialized it only once at the DAG level.
            if exec_id:
                task_data["execution_id"] = exec_id

            # Older checkpoints stored retry counters as top-level fields.
            # Move them back into NodeMetrics so TaskNode validation succeeds.
            legacy_metrics = dict(task_data.get("metrics") or {})
            retry_count = task_data.pop("retry_count", None)
            max_retries = task_data.pop("max_retries", None)
            if retry_count is not None and "retry_count" not in legacy_metrics:
                legacy_metrics["retry_count"] = retry_count
            if max_retries is not None and "max_retries" not in legacy_metrics:
                legacy_metrics["max_retries"] = max_retries
            if legacy_metrics:
                task_data["metrics"] = legacy_metrics
