...
class TaskNode(BaseModel):
    ...
    context_input: Optional[str] = Field(
        default=None,
        description="Planner-provided local guidance, evidence focus, or task brief",
    )
    dynamic_prompt: Optional[str] = Field(
        default=None,
        description=(
            "Task-specific dynamic prompt generated by the planner for this node. "
            "Contrasts with static prompts: dynamic_prompt is tailored per goal/input "
            "to guide the executor toward better output quality."
        ),
    )
