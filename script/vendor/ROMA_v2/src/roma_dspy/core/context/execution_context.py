...
class ExecutionContext:
    ...
    _current_task_id: ContextVar[Optional[str]] = ContextVar(
        "execution_context_current_task_id", default=None
    )
    ...
    @classmethod
    def set_current_task(cls, task_id: str) -> "ContextVar.Token":
        """Bind the current task ID to the active async execution scope."""
        return cls._current_task_id.set(task_id)

    @classmethod
    def reset_current_task(cls, token: "ContextVar.Token") -> None:
        """Restore the previous task binding for the active async scope."""
        cls._current_task_id.reset(token)

    @classmethod
    def get_current_task_id(cls) -> Optional[str]:
        """Return the task ID bound to the current async execution scope."""
        return cls._current_task_id.get()

    @classmethod
    def resolve_artifact_creator(
        cls, fallback_task_id: Optional[str] = None
    ) -> Optional[str]:
        """Resolve the best available task ID for artifact provenance."""
        current_task_id = cls.get_current_task_id()
        if current_task_id:
