...
class FundamentalContext(BaseModel):
    ...
    output_language: str = Field(
        default="en",
        description=(
            "Required output language for ALL agents in this execution. "
            "Detected once from the script of overall_objective and locked for the entire pipeline. "
            "Possible values: 'en' (English), 'zh' (Simplified Chinese). "
            "Every agent MUST write its output in this language regardless of topic domain."
        ),
        examples=["en", "zh"],
    )
    ...
    def to_xml(self) -> str:
        ...
        xml_parts = [
            ...
            f"  <output_language>{self.output_language}</output_language>",
...
class ExecutorSpecificContext(BaseModel):
    ...
    task_context_input: Optional[str] = Field(
        default=None,
        description="Planner-provided task brief, evidence focus, or local guidance",
    )
    ...
    parent_write_scope: Optional[str] = Field(
        default=None,
        description=(
            "When this executor is a THINK child of a WRITE parent, carries "
            "the parent's goal text as a hard scope constraint. The THINK "
            "executor MUST NOT generate chapters or content outside this scope."
        ),
    )
    report_policy: Optional[str] = Field(
        default=None,
        description=(
            "Global report strategy (audience, tone, length targets) from "
            "the root Planner. Ensures THINK outlines maintain consistent "
            "quality and style constraints across the report."
        ),
    )
    evidence_inventory: Optional[str] = Field(
        default=None,
        description=(
            "Compact inventory of ALL evidence .md files in the execution "
            "with descriptions. Built from the full artifact registry (not "
            "just dependency-scoped artifacts) so nested THINK tasks can see "
            "retrieve_*.md files even when they are not direct dependencies."
        ),
    )
    mandate_checklist: Optional[str] = Field(
        default=None,
        description=(
            "Pre-formatted <mandate_checklist>...</mandate_checklist> XML block "
            "containing per-WRITE hard constraints extracted from the user query by "
            "the Outline THINK executor. Two item types: "
            "'verbatim' (strings that must appear character-for-character) and "
            "'dimension' (analytical angles that must each have a dedicated paragraph). "
            "Injected as raw XML — not escaped — so the WRITE executor can parse it."
        ),
    )
    ...
    def to_xml(self) -> str:
        ...
        has_content = (
            ...
            or self.available_artifacts
            or self.task_context_input
            or self.parent_write_scope
            or self.report_policy
            or self.evidence_inventory
            or self.mandate_checklist
        )
        if not has_content:
        ...
        if self.parent_write_scope:
            xml_parts.append("  <parent_write_scope>")
            xml_parts.append(f"    {self._escape_xml(self.parent_write_scope)}")
            xml_parts.append("  </parent_write_scope>")

        if self.report_policy:
            xml_parts.append("  <report_policy>")
            xml_parts.append(f"    {self._escape_xml(self.report_policy)}")
            xml_parts.append("  </report_policy>")

        if self.task_context_input:
            xml_parts.append(
                "  <task_context_input>"
                f"{self._escape_xml(self.task_context_input)}"
                "</task_context_input>"
            )

        if self.evidence_inventory:
            xml_parts.append("  <evidence_inventory>")
            xml_parts.append(f"    {self._escape_xml(self.evidence_inventory)}")
            xml_parts.append("  </evidence_inventory>")
        ...
        # Mandate checklist — injected as raw XML (not escaped) so the WRITE
        # executor can read individual <verbatim> / <dimension> child elements.
        if self.mandate_checklist:
            xml_parts.append(f"  {self.mandate_checklist.strip()}")
...
class ParentResult(BaseModel):
    ...
    task_type: Optional[str] = Field(
        default=None,
        description=(
            "task_type of the parent node (RETRIEVE/THINK/WRITE/None). "
            "Constrains which child task_types are valid for this decomposition."
        ),
    )
...
class PlannerSpecificContext(BaseModel):
    ...
    blueprint_context: Optional[str] = Field(
        default=None,
        description=(
            "PlanBlueprint context injected from MIPE evolution. "
            "Full blueprint for Root Planner (depth=0), node-scoped "
            "context for Sub-Planner (depth>=1 with blueprint_node_id)."
        ),
    )
    report_policy: Optional[str] = Field(
        default=None,
        description=(
            "Global report strategy from root Planner (audience, tone, total length, "
            "depth allocation, evidence strategy). Injected into sub-Planner context "
            "so all decomposition levels maintain consistent output quality constraints."
        ),
    )
    global_outline: Optional[ArtifactReference] = Field(
        default=None,
        description=(
            "The report_outline.md artifact produced by the sibling THINK task. "
            "Injected at highest priority so the WRITE sub-planner anchors on the "
            "full heading skeleton before processing any other artifacts or sibling text."
        ),
    )
    global_outline_content: Optional[str] = Field(
        default=None,
        description=(
            "Full markdown body loaded from report_outline.md. Unlike metadata.preview, "
            "this field is not truncated and should be treated as the authoritative "
            "heading skeleton for downstream WRITE decomposition."
        ),
    )

    def to_xml(self) -> str:
        """Serialize parent, sibling context, artifacts, and blueprint to XML."""
        ...
        if self.blueprint_context:
            xml_parts.append("  <blueprint_guidance>")
            xml_parts.append(f"    {self._escape_xml(self.blueprint_context)}")
            xml_parts.append("  </blueprint_guidance>")

        if self.global_outline:
            xml_parts.append("  <global_heading_skeleton>")
            xml_parts.append("    <artifact_ref>")
            xml_parts.append(
                f"      {self._escape_xml(self.global_outline.to_xml_element())}"
            )
            xml_parts.append("    </artifact_ref>")
            if self.global_outline_content:
                xml_parts.append("    <full_outline_markdown>")
                xml_parts.append(
                    f"      {self._escape_xml(self.global_outline_content)}"
                )
                xml_parts.append("    </full_outline_markdown>")
            xml_parts.append("  </global_heading_skeleton>")
        ...
        if self.parent_results:
            ...
            for parent in self.parent_results:
                ...
                if parent.task_type:
                    xml_parts.append(f"      <task_type>{parent.task_type}</task_type>")
                xml_parts.append(
...
class AggregatorSpecificContext(BaseModel):
    ...
    - Evidence inventory: Compact listing of all retrieve_*.md files with descriptions,
      so RETRIEVE aggregators can reference real filenames when building the evidence catalog.
    ...
    evidence_inventory: Optional[str] = Field(
        default=None,
        description=(
            "Compact inventory of ALL evidence .md files registered so far, "
            "each with a short description. Injected for RETRIEVE aggregators so "
            "they can reference real retrieve_*.md filenames in the evidence catalog."
        ),
    )

    def to_xml(self) -> str:
        """Serialize to XML for aggregator consumption."""
        has_content = self.available_artifacts or self.evidence_inventory
        if not has_content:
            return (
        ...
        if self.evidence_inventory:
            xml_parts.append("  <evidence_inventory>")
            xml_parts.append(f"    {FundamentalContext._escape_xml(self.evidence_inventory)}")
            xml_parts.append("  </evidence_inventory>")
