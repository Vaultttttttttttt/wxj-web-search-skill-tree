...
class BaseModule(dspy.Module):
    ...
    def forward(
        ...
        **call_kwargs: Any,
        ...
        Args:
            ...
            directive: Optional high-priority task directive kept separate from
                general context/evidence payloads.
    ...
    async def _get_execution_tools(self) -> Dict[str, Any]:
        ...
        # User configs can override mandatory toolkits by specifying the same class_name
        user_configs = list(self._toolkit_configs or [])
        user_class_names = {cfg.class_name for cfg in user_configs}
        filtered_mandatory = [
            cfg for cfg in mandatory_configs
            if cfg.class_name not in user_class_names
        ]
        all_configs = filtered_mandatory + user_configs
