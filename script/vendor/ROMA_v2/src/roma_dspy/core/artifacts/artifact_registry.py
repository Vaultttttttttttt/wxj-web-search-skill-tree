...
class ArtifactRegistry:
    ...
    def _add_to_indexes(self, artifact: Artifact) -> None:
        """Insert an artifact into task/type/media indexes."""
    ...
    def _remove_from_indexes(self, artifact: Artifact) -> None:
        """Remove an artifact from task/type/media indexes if present."""
        artifact_id = artifact.artifact_id

        self._task_index.get(artifact.created_by_task, set()).discard(artifact_id)
        self._type_index.get(artifact.artifact_type, set()).discard(artifact_id)
        self._media_index.get(artifact.media_type, set()).discard(artifact_id)

    def _reindex_merged_artifact(self, existing: Artifact, merged: Artifact) -> None:
        """Refresh indexes after deduplicating a path into a merged artifact."""
        if (
            existing.created_by_task == merged.created_by_task
            and existing.artifact_type == merged.artifact_type
            and existing.media_type == merged.media_type
        ):
            return

        self._remove_from_indexes(existing)
        self._add_to_indexes(merged)
    ...
    async def register(self, artifact: Artifact) -> Artifact:
        ...
        async with self._lock:
            ...
            if existing_id:
                ...
                self._reindex_merged_artifact(existing, merged)
