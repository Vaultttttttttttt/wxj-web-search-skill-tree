...
def _search_tavily(keyword: str, limit: Optional[int], **kwargs) -> List[Dict]:
    ...
    cmd = [sys.executable, str(script_path), keyword, "--json"]
    search_depth = kwargs.get("search_depth") or os.getenv("TAVILY_SEARCH_DEPTH")
    if search_depth:
        cmd.extend(["--search-depth", str(search_depth)])
    if kwargs.get("include_answer") or os.getenv("TAVILY_INCLUDE_ANSWER", "").lower() in ("1", "true", "yes"):
...
def _search_rss(keyword: str, limit: Optional[int], **kwargs) -> List[Dict]:
    ...
    if limit is not None:
        cmd.extend(["-l", str(limit)])
    feeds_file = Path(__file__).parent.parent / "rss_search" / "rss_feeds.txt"
    if feeds_file.exists():
        cmd.extend(["--feeds", str(feeds_file), "--max-feeds", "8", "--timeout", "8"])
